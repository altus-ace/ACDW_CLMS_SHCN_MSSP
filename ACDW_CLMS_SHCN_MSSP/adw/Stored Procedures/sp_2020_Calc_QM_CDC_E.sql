...
BEGIN TRAN
	...
	SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Diabetic Retinal Screening', '', '','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a
    JOIN			[adw].[2020_tvf_Get_ProvSpec](18, 41, '', '', '', '') c 
	ON				a.SEQ_CLAIM_ID = c.SEQ_CLAIM_ID;
	--prior year negative result from screening by professional
    INSERT INTO		@TmpTable1(SUBSCRIBER_ID)
    SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Diabetic Retinal Screening', '', '','', CONCAT('1/1/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear)) a
    JOIN
					(
					SELECT DISTINCT SEQ_CLAIM_ID
					FROM [adw].[2020_tvf_Get_ClaimsByValueSet]('', 'Diabetic Retinal Screening Negative', '','', CONCAT('1/1/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear))
					) b 
	ON				a.SEQ_CLAIM_ID = b.SEQ_CLAIM_ID
    JOIN			[adw].[2020_tvf_Get_ProvSpec](18, 41, '', '', '', '') c 
	ON				a.SEQ_CLAIM_ID = c.SEQ_CLAIM_ID;
    ...
    JOIN
					(	SELECT DISTINCT SEQ_CLAIM_ID
				    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('', 'Diabetes Mellitus Without Complications', '','', CONCAT('1/1/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear))
					) b 
	ON				a.SEQ_CLAIM_ID = b.SEQ_CLAIM_ID
    JOIN			[adw].[2020_tvf_Get_ProvSpec](18, 41, '', '', '', '') c 
	ON				a.SEQ_CLAIM_ID = c.SEQ_CLAIM_ID;
    ...
    INSERT INTO		@TmpTable1(SUBSCRIBER_ID)
    SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Diabetic Retinal Screening With Eye Care Professional', '', '','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a;

    --prior year diabetes ret screening with eye care professional value code with negative result
    INSERT INTO		@TmpTable1(SUBSCRIBER_ID)
    SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Diabetic Retinal Screening With Eye Care Professional', '', '','', CONCAT('1/1/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear)) a
    JOIN
					(
					SELECT DISTINCT SEQ_CLAIM_ID
					FROM [adw].[2020_tvf_Get_ClaimsByValueSet]('', 'Diabetic Retinal Screening Negative', '','', CONCAT('1/1/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear - 1), CONCAT('12/31/', @MeasurementYear))
    ...
    INSERT INTO		@TmpTable1(SUBSCRIBER_ID)
    SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Diabetic Retinal Screening Negative', '', '','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a;
	--unilateral eye enucleation with a bilateral modifier
    INSERT INTO		@TmpTable1(SUBSCRIBER_ID)
    SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Unilateral Eye Enucleation', '', '','', CONCAT('1/1/', @MeasurementYear - 100), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a
    JOIN
					(
					SELECT DISTINCT SEQ_CLAIM_ID
					FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Bilateral', '', '', '',CONCAT('1/1/', @MeasurementYear - 100), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
					) b 
	ON				a.SEQ_CLAIM_ID = b.SEQ_CLAIM_ID;
    ...
    JOIN	
					(
					SELECT DISTINCT  *
					FROM			 [adw].[2020_tvf_Get_ClaimsByValueSet]('Unilateral Eye Enucleation', '', '','', CONCAT('1/1/', @MeasurementYear - 100), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
					) b 
	ON				a.SUBSCRIBER_ID = b.SUBSCRIBER_ID
    ...
    JOIN
					(
					SELECT DISTINCT *
					FROM [adw].[2020_tvf_Get_ClaimsByValueSet]('Unilateral Eye Enucleation Left', '', '','', CONCAT('1/1/', @MeasurementYear - 100), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
					) b 
	ON				a.SUBSCRIBER_ID = b.SUBSCRIBER_ID
    AND				a.SEQ_CLAIM_ID <> b.SEQ_CLAIM_ID
    AND				ABS(DATEDIFF(day, a.PRIMARY_SVC_DATE, b.PRIMARY_SVC_DATE)) >= 14;
    ...
    JOIN
					(
					SELECT DISTINCT  *
					FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Unilateral Eye Enucleation Right', '', '', '',CONCAT('1/1/', @MeasurementYear - 100), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
					) b 
	ON				a.SUBSCRIBER_ID = b.SUBSCRIBER_ID
    AND				a.SEQ_CLAIM_ID <> b.SEQ_CLAIM_ID
    AND				ABS(DATEDIFF(day, a.PRIMARY_SVC_DATE, b.PRIMARY_SVC_DATE)) >= 14;
    ...
    INSERT INTO		@TmpTable1(SUBSCRIBER_ID)
    SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Unilateral Eye Enucleation Left', '', '','', CONCAT('1/1/', @MeasurementYear - 100), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a
    JOIN
					(
						SELECT DISTINCT *
						FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Unilateral Eye Enucleation Right', '', '','', CONCAT('1/1/', @MeasurementYear - 100), CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
...
/***
Usage: 
EXEC [adw].[sp_2020_Calc_QM_CDC_E] '[adw].[QM_ResultByMember_HISTORY]',2019,16
***/
