CREATE PROCEDURE [adw].[p_PdwMbr_UpdateFctMembershipExcluded_Stg]( @EffectiveDate DATE)
AS 
    SET NOCOUNT ON
    SET XACT_ABORT ON
  /* Load BENEX Process  
    ...
    */
    ...
    BEGIN TRY
	   ...
	   OUTPUT INSERTED.MbrStg2_MbrDatakey INTO @Output
	   FROM ast.MbrBenExclusion_Working w
		  JOIN ast.MbrStg2_MbrData mbr 
    		  ON w.MedicareBeneficiaryID = mbr.ClientMemberKey	
	   	 AND @EffectiveDate BETWEEN mbr.RwEffectiveDate and mbr.RwExpirationDate 
	   ;
    COMMIT TRAN UpdateFctMemberBenEx        
    END TRY
    BEGIN CATCH
