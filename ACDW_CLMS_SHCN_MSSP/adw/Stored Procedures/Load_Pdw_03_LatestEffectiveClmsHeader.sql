...
AS 
    ...
    SELECT @InpCnt = COUNT(*) 
		FROM (SELECT csk.clmSKey, ch.MSSPPartAClaimKey
				...
				JOIN adi.Steward_MSSPPartAClaim ch ON csk.PRVDR_OSCAR_NUM = ch.CMSCertificationNBR
					...
					AND csk.ClaimTypeCode = ch.ClaimTypeCD
    ...
    INSERT INTO [ast].[ClaimHeader_03_LatestEffectiveClaimsHeader]
    ...
    MERGE [ast].[ClaimHeader_03_LatestEffectiveClaimsHeader] TRG
    USING(    SELECT stg.clmSKey
			 FROM (    SELECT Stg.clmSKey, MAX(stg.ClaimAdjCode) MaxAdjCode
					   FROM ast.[ClaimHeader_03_LatestEffectiveClaimsHeader] stg
