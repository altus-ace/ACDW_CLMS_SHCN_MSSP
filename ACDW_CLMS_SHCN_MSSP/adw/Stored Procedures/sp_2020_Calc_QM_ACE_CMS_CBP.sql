...
-- =============================================
CREATE PROCEDURE [adw].[sp_2020_Calc_QM_ACE_CMS_CBP] 
...
BEGIN TRAN
	...
	JOIN			[adw].[2020_tvf_Get_ClaimsByValueSet]('', ''
					,'','HEDIS_ACO_Essential Hypertension', @PriorYearBegin, @CurrentYearJun, @CodeSetEffective) b
	...
	INSERT INTO		 @TmpTable2(SUBSCRIBER_ID)
	SELECT DISTINCT  a.SUBSCRIBER_ID--,'0','0','1900-01-01'
    FROM			 [adw].[2020_tvf_Get_ActiveMembers](@MbrAceEffectiveDate) a	 
	JOIN			
					(
						SELECT DISTINCT	o.SUBSCRIBER_ID
	...
	--Generating and Calculating Values for Exclusions for ages 65 and above
	UNION
    ...
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('HEDIS_ACO_Systolic Less Than 140', '', '', ''
					, @PrimSvcDate_Start, @PrimSvcDate_End, @CodeSetEffective) a
	JOIN			[adw].[2020_tvf_Get_ClaimsByValueSet]('HEDIS_ACO_Diastolic Less Than 80', '', '', ''
    ...
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('HEDIS_ACO_Systolic Less Than 140', '', '', ''
					, @PrimSvcDate_Start, @PrimSvcDate_End, @CodeSetEffective) a
	JOIN			[adw].[2020_tvf_Get_ClaimsByValueSet]('HEDIS_ACO_Diastolic 80-89', '', '', ''
					, @PrimSvcDate_Start, @PrimSvcDate_End, @CodeSetEffective)	b
	ON				a.SUBSCRIBER_ID = b.SUBSCRIBER_ID
...
/***
Usage: 
EXEC [adw].[sp_2020_Calc_QM_ACE_CMS_CBP]	 @ConnectionStringProd	= '[adw].[QM_ResultByMember_History]',
											 @QMDATE				= '2021-05-15',
