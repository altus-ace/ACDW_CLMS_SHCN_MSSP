...
BEGIN TRAN
    ...
    SELECT DISTINCT SUBSCRIBER_ID
  	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Pregnancy', 'Sexual Activity', 'Pregnancy Tests','', CONCAT('1/1/', @MeasurementYear),CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a
	UNION
	SELECT DISTINCT	SUBSCRIBER_ID
	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Contraceptive Medications', '', '', '',CONCAT('1/1/', @MeasurementYear),CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) b 
	...
	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Chlamydia Tests','','','',CONCAT('1/1/', @MeasurementYear),CONCAT('12/31/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
...
/***
Usage: 
EXEC [adw].[sp_2020_Calc_QM_CHL] '[adw].[QM_ResultByMember_HISTORY]',2019,16
***/
