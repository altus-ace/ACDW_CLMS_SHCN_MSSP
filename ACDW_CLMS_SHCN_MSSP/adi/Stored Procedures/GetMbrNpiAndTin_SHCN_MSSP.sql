...
CREATE PROCEDURE [adi].[GetMbrNpiAndTin_SHCN_MSSP] (@DataDate DATE, @RowStatus INT,@ClientKey INT)

AS
					-- DECLARE @DataDate DATE = '2021-03-13' DECLARE @RowStatus INT = 0 DECLARE @ClientKey INT = 16
					...
					SELECT	pr.NPI
							,pr.AttribTIN
							,src.AttributedNPI					AS	ClientNPI
							,''							AS	ClientTIN 
							,src.MBI_ID 
					FROM		(SELECT		src.MBI_ID,src.AttributedNPI
								 FROM		adi.[MSSPPatientAttributionList] src
								 WHERE		DataDate =  @DataDate 
								 AND		RowStatus =  @RowStatus 
								) src
					LEFT JOIN	(SELECT		* 
								 FROM		[ACECAREDW].adw.tvf_AllClient_ProviderRoster (16,GETDATE(),1)
								 )pr
					ON			pr.NPI = src.AttributedNPI -- order by memberid --- 1326209354
					AND			pr.AttribTIN = ''
							...
							--Step 2 
								--Step 3 
									...
									LEFT JOIN	(SELECT		NPI AS prNPI,AttribTIN AS prTIN
												 FROM		ACECAREDW.[adw].[tvf_AllClient_ProviderRoster_TinRank](16,GETDATE(),1) 
												 ) a
